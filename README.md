...
The only thing you have to do is to decribe a [serveMux](https://pkg.go.dev/net/http#ServeMux) with its route handlers list. Then run the `httpmulti` server with this serveMux. As for simple `net/http` library the server starts with either function
